...
class UserController extends Controller
{
    ...
    public function details($id)
    {
        $membre = User::find($id);
        $equipes = Equipe::all();
        $contactUser = ContactUser::all();
        $roles = Role::all();
        $labo = Parametre::find('1');
        $materiels = DB::select('SELECT categories.nom 
            FROM categories,materiels
            WHERE  materiels.user_id= ? AND materiels.categorie_id=categories.id ',
            [$id]);
     ...
     public function detailsF($id)
    {
        ...
        $equipes = Equipe::all(); 
        $roles = Role::all();
        $projets = DB::select('SELECT projets.intitule FROM projets,projet_user,users WHERE users.id=? AND projet_user.user_id=? AND projets.id=projet_user.projet_id 
            ',[$id,$id]);

        return view('frontO.details_membre')->with([
            'membre' => $membre,
            'equipes' => $equipes,
            'roles' => $roles,
            'projets'=> $projets
    ...
    public function trombin()
    {
        // $membres = User::all()->orderBy('name'); 
       
        $membres = DB::table('users')->distinct('id')->orderBy('name')->get(); 

        return view('frontO.trombinoscope', ['membres' => $membres]);
    } 
       ...
       public function listt()
    {
        ...
        return view('frontO.liste_membres' , ['membres' => $membres]);
    }
